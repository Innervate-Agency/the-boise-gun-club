...
const badgeVariants = cva(
  "inline-flex items-center justify-center rounded-full border font-medium w-fit whitespace-nowrap shrink-0 [&>svg]:pointer-events-none [transition:var(--transition-fast)] relative overflow-hidden",
  {
    variants: {
      variant: {
        ...
        warning: "border-(--color-leonard-yellow)/50 bg-(--color-leonard-yellow)/10 text-(--color-leonard-yellow) hover:bg-(--color-leonard-yellow)/20 hover:border-(--color-leonard-yellow)/70 dark:border-(--color-leonard-yellow)/50 dark:bg-(--color-leonard-yellow)/10 dark:text-(--color-leonard-yellow) dark:hover:bg-(--color-leonard-yellow)/20",
        ...
        premium: "bg-gradient-to-r from-(--color-leonard-yellow) to-(--color-lahoma-orange) text-black [box-shadow:var(--shadow-md)] border border-(--color-leonard-yellow)/20 hover:[box-shadow:var(--shadow-lg)] hover:scale-[1.05] [transition:var(--transition-smooth)] relative overflow-hidden group motion-reduce:hover:scale-100",
        elite: "bg-gradient-to-r from-(--color-leonard-yellow) via-(--color-lahoma-orange) to-(--color-leonard-yellow) bg-[length:200%_100%] text-black [box-shadow:var(--shadow-lg)] border-2 border-(--color-leonard-yellow)/30 animate-shimmer hover:[box-shadow:var(--shadow-xl)] hover:scale-[1.10] [transition:var(--transition-smooth)] relative overflow-hidden group motion-reduce:animate-none motion-reduce:hover:scale-100",
...
function Badge({
  ...
  const badgeContent = (
    ...
    >
      ...
      {variant === 'premium' && (
        <div className="absolute inset-0 bg-gradient-to-r from-(--color-lahoma-orange)/30 to-(--color-leonard-yellow)/30 blur-md opacity-50 group-hover:opacity-70 [transition:var(--transition-smooth)] -z-10" />
      )}
      
      {/* Elite badge shimmer effect */}
      {variant === 'elite' && (
        <div className="absolute inset-0 bg-gradient-to-r from-(--color-lahoma-orange)/40 to-(--color-leonard-yellow)/40 blur-lg opacity-60 group-hover:opacity-80 [transition:var(--transition-smooth)] -z-10" />
      )}
      ...
      {dismissible && onDismiss && (
        <button
          type="button"
          onClick={onDismiss}
          className="ml-1 flex-shrink-0 hover:bg-primary/10 rounded-full p-0.5 [transition:var(--transition-fast)]"
        >
